...
export async function POST(req: NextRequest) {
  ...
  if (!sendRes?.ok) {
    const err = await sendRes?.json().catch(() => null) as { error?: string } | null;
    return NextResponse.json(
      { error: err?.error ?? "Could not reach agent bridge. Is agent_bridge.py running?" },
      { status: 502 }
    );
  }
  ...
  return NextResponse.json(
    { error: "Agent did not reply within 30 seconds." },
    { status: 504 }
  );
}
